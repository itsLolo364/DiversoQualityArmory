...
public class QAListener implements Listener {
	...
	private final List<String> unloadingPlayers = new ArrayList<>();
	...
	public void onDamage(QADamageEvent event) {
		...
		String info = "§aɪɴꜰᴏʀᴍᴀᴢɪᴏɴɪ ѕᴘᴀʀᴏ\n\n§7Arma: §f" + gun.getName() + "\n§7Danno: §f" +
				DECIMAL_FORMAT.format(event.getDamage()) + "\n\n§7Posizione " + damager.getName() +
				": §f" + damager.getLocation().getBlockX() + ", " + damager.getLocation().getBlockY() +
				", " + damager.getLocation().getBlockZ() + "\n§7Posizione " + damaged.getName() + ": §f" +
				damaged.getLocation().getBlockX() + ", " + damaged.getLocation().getBlockY() + ", " +
				damaged.getLocation().getBlockZ() + "\n\n§eClicca per teletrasportarti\n";

		Component text = text("§2[SPARO] §a" + damager.getName() + "§7 ha sparato a §a" + damaged.getName()
				+ " §7con §a" + gun.getName() + ".")
	...
	public void onHit(EntityDamageByEntityEvent e) {
		...
		if (e.getDamager() instanceof Player damager) {
			if ((e.getCause() == DamageCause.ENTITY_ATTACK || e.getCause() == DamageCause.ENTITY_SWEEP_ATTACK)) {
				if (QualityArmory.isMisc(damager.getItemInHand())) {
					CustomBaseObject aa = QualityArmory.getMisc(damager.getItemInHand());
				...
				if (e.getEntity() instanceof Player) {
					...
					if (!QualityArmory.isCustomItem(damager.getItemInHand())) {
						ItemStack offhand = Update19OffhandChecker.getItemStackOFfhand(damager);
						...
						if (QualityArmory.isIronSights(damager.getItemInHand())) {
							object = QualityArmory.getCustomItem(damager.getInventory().getItemInOffHand());
						} else {
							object = QualityArmory.getCustomItem(damager.getItemInHand());
						}
					}
				...
				if (QAMain.allowGunHitEntities && !ignoreClick.contains(damager.getUniqueId())) {
					...
					QACustomItemInteractEvent event = new QACustomItemInteractEvent(damager, g);
					...
					Bukkit.getScheduler().runTaskLater(QAMain.getInstance(), () -> {
						finalG.onLMB(damager, damager.getItemInHand());
						ignoreClick.remove(damager.getUniqueId());
					}, 1L);
				}
			}
		}
	...
	public void onShift(final PlayerToggleSneakEvent e) {
		...
		if (!QAMain.enableIronSightsON_RIGHT_CLICK) {
			...
			try {
				if (e.isSneaking()) {
					ItemStack itemInMainHand = e.getPlayer().getItemInHand();
					ItemStack itemInOffHand = e.getPlayer().getInventory().getItemInOffHand();

					if (QualityArmory.isGun(itemInMainHand)) {
						DEBUG("Is Sneaking with gun in main hand");
						if ((QualityArmory.isCustomItem(itemInOffHand)) && !QualityArmory.isIronSights(itemInOffHand)) {
							CustomBaseObject offhandObj = QualityArmory.getCustomItem(itemInOffHand);
	...
	public void onPlayerDeath(PlayerDeathEvent event) {
		...
		String nomeArma = arma.hasItemMeta() && arma.getItemMeta().hasDisplayName()
				? arma.getItemMeta().getDisplayName()
				: arma.getType().name();

		String info = "§cɪɴꜰᴏʀᴍᴀᴢɪᴏɴɪ ᴍᴏʀᴛᴇ\n\n"
				+ "§7Killer: §f" + killer.getName() + "\n"
				+ "§7Vittima: §f" + morto.getName() + "\n"
				+ "§7Arma: §f" + nomeArma + "\n\n"
				+ "§7Posizione Killer: §f"
				+ killer.getLocation().getBlockX() + ", "
				+ killer.getLocation().getBlockY() + ", "
				+ killer.getLocation().getBlockZ() + "\n"
				+ "§7Posizione Vittima: §f"
				+ morto.getLocation().getBlockX() + ", "
				+ morto.getLocation().getBlockY() + ", "
				+ morto.getLocation().getBlockZ() + "\n\n"
				+ "§eClicca per teletrasportarti";

		Component messaggio = Component.text(
				"§4[MORTE] §c" + killer.getName()
						+ "§7 ha ucciso §c" + morto.getName()
						+ " §7con §c" + nomeArma + ".")
				.clickEvent(ClickEvent.runCommand("/tp " + killer.getName()))
				.hoverEvent(HoverEvent.showText(Component.text(info)));

		Bukkit.getOnlinePlayers().forEach(p -> p.sendMessage(messaggio));
	}
	...
	public void onPlayerChangeSlot(PlayerItemHeldEvent event) {
		...
		player.sendMessage(text("\n §cɪɴᴛᴇʀʀᴏᴛᴛᴏ \n §7Hai interrotto lo scaricamento dell'arma. \n"));
	}
	...
	public void onPlayerDrop(PlayerDropItemEvent event) {
		...
		Bukkit.getScheduler().runTaskLater(QAMain.getInstance(), () -> {
			...
			QualityArmory.addAmmoToInventory(player, gun.getAmmoType(), amount);
			Gun.updateAmmo(gun, player, 0);
